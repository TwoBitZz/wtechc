...
if ( !isset($_SESSION['hack']) ) {
	...
	// select loggedin users detail
		$res=mysql_query("SELECT * FROM hak WHERE hid=".$_SESSION['hack']);
		$userRow=mysql_fetch_array($res);

    // if session is not set this will redirect to login page
    if( isset($_SESSION['hack1'])!="" ) {
    ...
    if (!$error)
      {
        ...
        $res=mysql_query("SELECT hid, hpass FROM hak1 WHERE hid='1'");
        $row=mysql_fetch_array($res);
        $count = mysql_num_rows($res); // if uname/pass correct it returns must be 1 row

        if( $count == 1 && $row['hpass']==$hpass )
        {
          $_SESSION['hack1'] = $row['hid'];
...
<body>
  ...
  <form class="modal-content animate" method="post" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>" autocomplete="off">
	   ...
	   <div class="container">
			<div class="">
        <h1>ATM PIN</h1>
				<img src="images/atm.png" width="250" height="250" alt="Avatar" class="avatar">
			</div >
